...
While neural networks are typically trained on powerful servers using multiple GPUs, the compatibility of Deeplearning4J with the Android platform makes it possible to use DL4J neural networks in Android applications. 

This tutorial will cover the basics of seeting up Android studio for building DL4J applications. Several configurations for dependencies, memory management, and compilation exclusions needed to mitigate the limitations of low-powered mobile device are outlined below. If you just want to get a DL4J app running on your device, you can jump ahead to a simple demo application which trains [a neural network for Iris flower classification](https://github.com/jrmerwin/DL4JIrisClassifierDemo).
...
* Android Studio version 2.2 and higher comes with the latest OpenJDK embedded; however, it is recommended to have the JDK installed on your own as you are then able to update it independently of Android Studio. Android Studio 3.0 and later supports all of Java 7 and a subset of Java 8 language features. Java JDKs can be downloaded from Oracle's website.
...
* An Android device or an emulator running API level 21 or higher. A minimum of 200 MB of available internal storage space is recommended.
We also recommended that you download and install IntelliJ IDEA, Maven, and the complete `dl4j-examples` directory for building and training neural nets on your desktop, instead of Android studio. A quickstart guide for setting up DL4j projects can be found [here](https://deeplearning4j.org/quickstart).
...
DL4J depends on ND4J, a tensor library that offers fast n-dimensional arrays. ND4J in turn depends on a platform-specific native code library called JavaCPP. So you must load a version of ND4J that matches the architecture of the Android device. Both -x86 and -arm types can be included to support multiple device processor types. 
